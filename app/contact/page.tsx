...
export default function ContactPage() {
  ...
  const contactItems = [
    { icon: <MapPin style={{ color: "#ff0080" }} size={22} />, title: t(tr.contact.address, lang), val: content.address,       glow: "rgba(255,0,128,0.2)" },
    { icon: <Phone style={{ color: "#00f5ff" }} size={22} />, title: t(tr.contact.phone,   lang), val: content.contactPhone,  glow: "rgba(0,245,255,0.2)" },
    { icon: <Mail  style={{ color: "#bf00ff" }} size={22} />, title: t(tr.contact.email,   lang), val: content.contactEmail,  glow: "rgba(191,0,255,0.2)" },
    { icon: <Clock style={{ color: "#00ff88" }} size={22} />, title: t(tr.contact.hours,   lang), val: t(tr.contact.hoursVal, lang), glow: "rgba(0,255,136,0.2)" },
  ];
    ...
    <div className="min-h-screen neon-bg relative overflow-hidden">
      ...
      <div className="py-16 px-4 relative z-10" style={{ background: "rgba(var(--bg-rgb), 0.4)", borderBottom: "1px solid rgba(0,245,255,0.1)" }}>
        <div className="max-w-4xl mx-auto text-center sm:text-left">
          <p className="text-sm font-semibold uppercase tracking-widest mb-2" style={{ color: "#00f5ff", textShadow: "0 0 10px rgba(0,245,255,0.4)" }}>
            {t(tr.contact.getInTouch, lang)}
          </p>
          <h1 className="font-display text-4xl sm:text-5xl font-bold text-[rgb(var(--fg-rgb))] tracking-wider">
            {t(tr.contact.pageTitle, lang)}
          </h1>
          <p className="mt-4 font-light text-[rgb(var(--muted-400-rgb))]">
            {t(tr.contact.pageSubtitle, lang)}
          </p>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className="bg-[var(--card-hex)]/60 backdrop-blur-md rounded-2xl p-8 border border-[rgba(0,245,255,0.15)] shadow-[0_0_20px_rgba(0,0,0,0.5)] h-full">
              <h2 className="font-display font-bold text-xl mb-8 text-[rgb(var(--fg-rgb))] tracking-wide border-b border-[rgba(var(--fg-rgb),0.05)] pb-4">
                {t(tr.contact.contactInfo, lang)}
              </h2>
              <div className="space-y-6">
            ...
            <div className="relative z-10">
              {sent ? (
                <div className="text-center py-20">
                  ...
                  <h3 className="font-display font-bold text-2xl mb-3 text-[rgb(var(--fg-rgb))] tracking-wider">
                    {t(tr.contact.sentTitle, lang)}
                  </h3>
                  <p className="text-[rgb(var(--muted-400-rgb))] text-sm mb-8 font-light">
                    {t(tr.contact.sentDesc, lang)}
                  </p>
                  <button onClick={() => setSent(false)} className="neon-btn-outline px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest">
                    {t(tr.contact.sendAnother, lang)}
                  </button>
                </div>
              ) : (
                <>
                  <h2 className="font-display font-bold text-xl mb-8 text-[rgb(var(--fg-rgb))] tracking-wide border-b border-[rgba(var(--fg-rgb),0.05)] pb-4">
                    {t(tr.contact.sendMsg, lang)}
                  </h2>
                  <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-5">
                    <div className="grid sm:grid-cols-2 gap-5">
                      {[
                        { key: "name",  label: t(tr.contact.fullName,  lang), placeholder: t(tr.contact.namePlaceholder,    lang) },
                        { key: "email", label: t(tr.contact.emailLabel, lang), placeholder: "your@email.com", type: "email" },
                      ].map((f) => (
                        <div key={f.key}>
                    ...
                    <div>
                      <label className="text-xs text-[rgb(var(--muted-400-rgb))] font-semibold uppercase tracking-widest mb-2 block">
                        {t(tr.contact.subject, lang)}
                      </label>
                      <input
                        required
                        value={form.subject}
                        ...
                        className="w-full bg-[rgba(var(--bg-rgb),0.3)] border border-[rgba(var(--fg-rgb),0.1)] rounded-xl px-4 py-3 text-sm text-[rgb(var(--fg-rgb))] outline-none placeholder-[rgb(var(--muted-500-rgb))] focus:border-[#00f5ff] focus:shadow-[0_0_15px_rgba(0,245,255,0.2)] transition-all"
                        placeholder={t(tr.contact.subjectPlaceholder, lang)}
                      />
                    </div>
                    <div>
                      <label className="text-xs text-[rgb(var(--muted-400-rgb))] font-semibold uppercase tracking-widest mb-2 block">
                        {t(tr.contact.message, lang)}
                      </label>
                      <textarea
                        required
                        rows={5}
                        ...
                        placeholder={t(tr.contact.messagePlaceholder, lang)}
                      />
                    </div>
                    <button type="submit" className="neon-btn-solid w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 uppercase tracking-widest text-sm mt-4">
                      <Send size={16} /> {t(tr.contact.sendBtn, lang)}
                    </button>
                  </form>
                </>
              )}
